    ...
    <Link href={`/story/${props.article.category}/${props.article.type}/${props.article.id}`}>
      <a target={'_blank'}>
        <div className={`${props.width || 'w-72 mr-5' } bg-white p-2 rounded-md inline-block `}>
          ...
          <div className='whitespace-normal mt-3 text-slate-600 text-base font-normal break-words' dangerouslySetInnerHTML={{__html: props.article?.title.slice(0, 60)}}></div>
