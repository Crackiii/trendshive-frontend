...
function Youtube(props: Props) {
  ...
  return (
    <Link href={`/story/${props.content.category || props.content.catgory}/${props.content.type}/${props.content.id}`}>
      <a>
        <div className={`${!showImage && width < 600 && 'grid grid-cols-12 gap-0'} bg-white rounded-lg group overflow-hidden font-light cursor-pointer hover:shadow-xl hover:shadow-zinc-200 transition-shadow`} >
          {
          ...
          {
            !showImage && width < 600 && 
            <div className='w-16 h-16 m-3 col-span-2 rounded-lg overflow-hidden shadow-lg'>
              <img src={image} alt='icon' className='object-cover min-h-full min-w-full' />
            </div>
          }
          <div className='px-3 pb-4 col-span-10'>
