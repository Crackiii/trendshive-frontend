      ...
      <Link href={`/story/${item.category}/${item.type}/${item.id}`} passHref>
          <a target={'_blank'}>
            ...
            <div className='absolute z-10 group top-0 left-0 w-full h-full bg-slate-900 bg-opacity-60 hover:bg-opacity-70 flex flex-col justify-end cursor-pointer overflow-hidden'>
              <div className='p-2'>
                <div className='text-lg text-white mb-2 group-hover:first:text-blue-400 group-hover:first:underline' dangerouslySetInnerHTML={{__html: item.title}}></div>
                <div className=' text-gray-300 text-xs font-light mb-2 tracking-wider' dangerouslySetInnerHTML={{__html: item.description.slice(0, 50)}}></div>
