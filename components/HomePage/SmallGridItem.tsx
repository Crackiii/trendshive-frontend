...
function SmallGridItem({item}: Props) {
  ...
  const {image} = useImage(item.images?.find(validURL) || favicon || item.all_images?.find(validURL))
      ...
      <span>
        <a className='mb-2 sm:mb-0 h-36 bg-white grid grid-cols-8 relative cursor-pointer overflow-hidden group'>
          <div className='relative col-span-2'>
            <img src={image} alt="img" className='object-cover w-full min-w-full h-full min-h-full' />
          </div>
