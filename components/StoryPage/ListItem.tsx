        ...
        <div className={`grid grid-cols-8 grid-rows-1 p-2 group relative cursor-pointer ${background ?? 'bg-white'}`}>
          <div className='col-span-2 flex'>
            <div className='w-16 h-16 relative overflow-hidden rounded-xl shadow-lg shadow-slate-400'>
              <img src={item?.imageUrl || favicon} alt='image' onError={(ev) => ev.currentTarget.src = favicon} className='min-h-full min-w-full object-cover' />
              {
                item?.isVideo && 
          ...
          <div className='col-start-3 col-span-6'>
            ...
            <div className='text-xs w-full overflow-hidden text-ellipsis whitespace-nowrap mt-2 text-slate-400'>{item?.description || item?.snippet}</div>
          </div>
